...
class APIsManager {
    ...
    func fetch20Images(completion: @escaping (Result<[ImageDataModel], Error>) -> Void) {
        ...
        guard let accessToken = accessToken else {
            completion(.failure(APIError.failedToGetAccessToken))
            return
        }
    ...
    func downloadImage(from urlString: String, imageId: String, completion: @escaping (Result<Data, Error>) -> Void) {
        guard let url = URL(string: urlString) else {
        ...
        let task = URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            if let data = data {
                ImageManager.shared.saveImage(imageName: imageId, imageData: data)
